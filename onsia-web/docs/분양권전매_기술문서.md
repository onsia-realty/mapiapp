...
11. [**로컬 테스트 및 Vercel 배포 가이드**](#11-로컬-테스트-및-vercel-배포-가이드) ⭐ NEW
12. [**API 적용 현황**](#12-api-적용-현황) ⭐ NEW
...
### 3.0 분양권 목록 조회 ⭐ NEW (2026-01-12)

```
GET /api/bunyanggwon/list
```

| 파라미터 | 타입 | 기본값 | 설명 |
|----------|------|--------|------|
| region | string | "경기" | 지역명 (서울, 경기, 인천 등) |
| page | number | 1 | 페이지 번호 |
| perPage | number | 20 | 페이지당 결과 수 |
  ...
  "data": [
    {
      ...
      "builder": "경남기업 주식회사",
      "homepageUrl": "https://...",
      "applicationStart": "20250113",
      "applicationEnd": "20250114"
    }
  ],
  "totalCount": 15,
  "page": 1,
...
**제외 조건** (비정상 매물):
- 블록, BL, 공공, 임대, 국민, 영구, 행복주택, LH, SH

**포함 조건** (정식 분양):
- 정식 지번 주소 (동, 번지, 일원 등)
- 브랜드 아파트 (자이, 래미안, 힐스테이트, 푸르지오, 더샵 등)

#### 소스 코드 위치
- `src/app/api/bunyanggwon/list/route.ts`
...
2. data.go.kr에서 API 키 발급
3. 환경 변수 설정 후 서버 재시작

---

#### 🔴 문제 2: 청약홈 API 빈 데이터 응답

**증상**: 목록에 데이터가 표시되지 않음

**원인**:
- 지역 파라미터 오류 ("경기도" vs "경기")
- API 요청 한도 초과
...
**증상**: 지도가 표시되지 않음, "카카오맵 SDK 로드 대기 중..."

**원인**:
- Kakao Developers에서 도메인 미등록
- API 키 오류

**해결**:
1. https://developers.kakao.com 접속
2. 앱 설정 → 플랫폼 → Web 도메인 등록
   - 로컬: `http://localhost:3000`
   - Vercel: `https://your-app.vercel.app`

---

#### 🔴 문제 4: Next.js Image 외부 도메인 오류

**증상**:
```
Error: Invalid src prop on `next/image`, hostname "xxx.co.kr" is not configured
```
...
|------|-------|-------------|
| `DATA_GO_KR_API_KEY` | `your_api_key` | Production, Preview, Development |
| `NEXT_PUBLIC_KAKAO_MAP_KEY` | `your_kakao_key` | Production, Preview, Development |
...
1. **API 키 노출 금지**
   ...
   - `DATA_GO_KR_API_KEY`는 서버 사이드에서만 사용
...
3. **API 호출 제한**
   - 공공데이터포털: 일 1,000회 (무료)
   - 청약홈 API: revalidate 3600초 캐싱 적용
...
| 페이지 | 경로 | API 연동 | Mock 사용 | 비고 |
|--------|------|----------|-----------|------|
| 분양권 목록 | `/category/bunyanggwon` | ✅ 청약홈 API | VIP 매물만 Mock | 2026-01-12 연동 완료 |
| 분양권 상세 | `/category/bunyanggwon/[id]` | ✅ 청약홈 API | fallback만 Mock | 청약 정보, 분양가, 일정 |
| 마피 매물 목록 | `/category/bunyanggwon/[id]/mapi` | ❌ | ✅ Mock | 향후 연동 예정 |
| 마피 매물 상세 | `/category/bunyanggwon/[id]/mapi/[mapiId]` | ❌ | ✅ Mock | 향후 연동 예정 |
...
| API | 용도 | 데이터 출처 | 캐싱 |
|-----|------|-------------|------|
| 청약홈 분양정보 | 분양 단지 목록/상세 | api.odcloud.kr | 1시간 |
| 청약홈 평형별 분양가 | 타입별 분양가 | api.odcloud.kr | 1시간 |
| 국토부 실거래가 | 주변 시세 | apis.data.go.kr | 1시간 |
| 지하철역 정보 | 주변 교통 | 정적 JSON | - |
| 학교 정보 | 학군 정보 | 정적 JSON | - |
| 버스 정류장 | 주변 교통 | api.odcloud.kr | 요청시 |

#### 🔄 연동 예정 API

| API | 용도 | 예정 시기 |
|-----|------|-----------|
| 온시아 매물 DB | 마피 매물 실데이터 | 백엔드 개발 후 |
| 금융감독원 금리 | 대출계산기 금리 | - |
| 호갱노노 시세 | 시세 그래프 | - |

---

### 12.3 데이터 흐름도 (2026-01-12 기준)

```
┌─────────────────────────────────────────────────────────────────────┐
│                    분양권 목록 페이지 (/category/bunyanggwon)          │
└─────────────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────────────┐
│  1. 청약홈 API 목록 조회 (/api/bunyanggwon/list)                      │
│     - 지역별 분양 단지 조회 (서울/경기/인천 등)                          │
│     - 필터링 로직 적용 (공공임대 제외, 브랜드 아파트만)                   │
└─────────────────────────────────────────────────────────────────────┘
                              │
          ┌───────────────────┴───────────────────┐
          ▼                                       ▼
┌─────────────────────────────┐   ┌─────────────────────────────┐
│  VIP 매물 (Mock 데이터)      │   │  분양 중인 단지 (API 데이터)  │
│  - 전매 매물 Mock            │   │  - 청약홈 실제 데이터         │
│  - 향후 온시아 DB 연동        │   │  - 필터링 후 표시            │
└─────────────────────────────┘   └─────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────────────┐
│  2. 상세 페이지 진입 시 (/category/bunyanggwon/[apiId])                │
│     - 청약홈 상세 API 호출                                            │
│     - 주변 시세 API (실거래가)                                         │
│     - 좌표 기반 주변 정보 (학교/지하철/버스)                             │
└─────────────────────────────────────────────────────────────────────┘
```

---

### 12.4 API 우선 원칙 (CLAUDE.md 정책)

> ⚠️ **중요**: Mock 데이터는 API 장애 시 fallback 또는 개발 참고용으로만 사용

```typescript
// ✅ 올바른 패턴: API 우선
const response = await fetch('/api/bunyanggwon/list?region=경기');
const result = await response.json();

if (result.success && result.data) {
  setApiData(result.data);  // API 데이터 우선 사용
} else {
  setApiData([]);           // API 실패 시 빈 배열
}

// ❌ 잘못된 패턴: Mock 우선
const data = mockBunyanggwon;  // Mock 데이터 직접 사용 금지
```
